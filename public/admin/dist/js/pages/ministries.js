  ...
  'use strict'
   var _token = $('meta[name="csrf-token"]').attr('content'); 
    $.ajaxSetup({
     ...
     var summernoteValidator =$("#MinistriesForm").validate({
        ...
        submitHandler: function (form) {
            ...
            $('#btnUpdateUser').attr('disabled', true);
             var formData = new FormData($('#MinistriesForm')[0]); 
             var txt = ($('#id').length)?'Update':'Create';
            $.ajax({
                    type: 'POST',
                    url: base_url+'/admin/ministries/save/',
                    data: formData,
                    dataType: 'json',
                    contentType: false,
                    cache: false,
     ...
     summernoteElement.summernote({
      ...
      var BLOGTABLE = $('#ministries-datatable').DataTable( {
        "scrollX": false,
         bLengthChange: false,
         responsive: true,
        "order": [[0, "desc" ]],
        "autoWidth": false,
                "processing": true,
                 "serverSide": true,
               "ajax": {
                    "url": base_url+"/admin/ministries/getdatatable",
                     "type": "POST",
                },
                "columns": [
                    { "data": "sno"},
   ...
   $('body').on('click','.view-desc',function(e){
      var id = $(this).attr('id'); 
       ...
       $.dialog({
                ...
                title:title,
                content: "url:"+base_url+"/admin/ministries/getDesc/"+id,
                type: 'red',
                animation: 'scale',
                columnClass: 'medium',
    ...
    $(document).on('click', '.btn-status', function (e) { 
        ...
        if(status=='Active'){
          st ='Inactive';typ="red";
          msg = "<p style='color:red;'>Sure you want to <i> Inactive ministries: </i> <strong>"+title+"</strong>? </p>";
        }else{
          st ='Active';typ="green";
          msg = "<p style='color:green;'>Sure you want to <i>Active ministries: </i> <strong>"+title+"</strong>? </p>";
        }
        ...
        $.confirm({
          ...
          buttons: {
              confirm: function () {
                $.get(base_url+"/admin/ministries/status/update/"+id+"/"+st,function(resp) {
     ...
     $(document).on('click', '.btn-delete', function (e) { 
        ...
        $.confirm({
          ...
          buttons: {
              confirm: function () {
                $.get(base_url+"/admin/ministries/delete/"+id,function(resp) {
                    if($.trim(resp.status)=='success'){
                         toastr.success(resp.message, 'Success');
                         BLOGTABLE.ajax.reload();
                    }else{
